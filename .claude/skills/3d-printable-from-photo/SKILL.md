...
sponge-holder design (`/3d-models/sponge-holder/` in this repo). Apply
to any new printable: clips, brackets, hooks, holders, jigs.
...
- **GitHub Actions** workflow on `push` to `3d-models/**` or
  `docs/**`: install Python deps, run **every** `generate_*.py` it can
  find, then `actions/upload-pages-artifact` + `actions/deploy-pages`.
  User must set `Settings → Pages → Source = "GitHub Actions"` (one
  click). When you add a new design, also add a step that runs its
  generator(s) — or better, glob all `3d-models/*/generate_stl.py`.
...
`3d-models/<design>/generate_stl.py`, update `/docs/index.html` to
include the new model. Concretely:
...
5. **Update the workflow** if your generator file is at a non-standard
   location. Otherwise, if the workflow uses
   `for f in 3d-models/*/generate_stl.py; do python3 "$f"; done`, it
   picks up new designs without edits.
6. Update the project's `Modelo NN` numbering on the eyebrow if you
   want, or just let the order in `<main>` define it.
...
`/3d-models/sponge-holder/` in this repo is the canonical example of
this skill applied end to end:
