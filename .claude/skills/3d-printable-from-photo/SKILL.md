...
└── models/
    ├── *.glb               # generated by generate_stl.py
    └── version.txt         # cache-buster timestamp

.github/workflows/pages.yml # regenerates GLBs in CI, deploys /docs to Pages
```
...
- **GitHub Actions** workflow on `push` to `3d-models/**` or
  `docs/**`: install Python deps, run all `generate_*.py`, then
